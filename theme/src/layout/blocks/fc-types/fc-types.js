...
document.addEventListener("DOMContentLoaded", function (event) {
	const typesTexts = document.querySelectorAll('.fc-types__text');

	if(typesTexts.length) {
		for (let i = 0; i < typesTexts.length; i++) {
			new SimpleBar( typesTexts[i], {
				autoHide: false,
			});
		}
	}

	const typesSlider = document.querySelector('.fc-types__container');

	if(typesSlider != null) {
		addSliderOnResize(typesSlider ,
			{
				slidesToShow: 1,
				infinite: false,
				slidesToScroll: 1,
