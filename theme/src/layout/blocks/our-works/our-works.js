...
document.addEventListener("DOMContentLoaded", function (event) {
	const ourWorksFilter = document.querySelector('.our-works__filter');

	if(ourWorksFilter !== null) {
		new SimpleBar( ourWorksFilter, {
			autoHide: false,
		});
	}

	const $ourWorksSlider = window.jQuery('.our-works__container');

	if($ourWorksSlider.length){

		addSliderOnResize($ourWorksSlider,
			{
				slidesToShow: 3,
				infinite: false,
				slidesToScroll: 1,
