...
window.jQuery(function () {
	...
	const $rPrices = window.jQuery('.r-prices');

	if($rPrices.length){

		window.jQuery('.r-prices__controls').on('click', 'li:not(.active)', function () {
			window.jQuery(this)
				.addClass('active').siblings().removeClass('active')
				.closest('div.r-prices__tabs').find('div.r-prices__item').removeClass('active')
				.eq(window.jQuery(this).index()).addClass('active')
				.find('.slick-slider').slick('refresh');
		});

		const $rProductsSlider = window.jQuery('.r-products');

		if($rProductsSlider.length){

			addSliderOnResize($rProductsSlider,
				{
					slidesToShow: 2,
					infinite: false,
					slidesToScroll: 1,
